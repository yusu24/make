...
/**
 * Standard utility helpers for Online Seller / E-Commerce PDF Reports
 */
...
interface SellerPrintHeaderProps {
  user?: any;
  title: string;
  subtitle?: string;
  docNumber?: string;
...
export const SellerPrintHeader: React.FC<SellerPrintHeaderProps> = ({
  user,
  title,
  subtitle,
  docNumber,
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2.5px solid #000000', paddingBottom: 14, marginBottom: 18 }}>
      <div>
        <h1 style={{ margin: 0, fontSize: 19, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', color: '#000000' }}>
          {user?.tenant_name || 'TOKO ONLINE & E-COMMERCE MULTI-CHANNEL'}
        </h1>
        <p style={{ margin: '4px 0 0', fontSize: 11, color: '#374151', lineHeight: 1.4 }}>
          {user?.address || 'Pusat Operasional Penjualan Online, Marketplace & Omnichannel'} {user?.phone ? `• Telp: ${user.phone}` : ''}
        </p>
        {subtitle && (
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        {docNumber && (
          <div style={{ margin: '4px 0 0', fontSize: 12, fontWeight: 700, color: '#4F46E5', fontFamily: 'monospace' }}>
            {docNumber}
          </div>
        )}
        ...
        {channelText && (
          <p style={{ margin: '2px 0 0', fontSize: 10, color: '#059669', fontWeight: 600 }}>
            Channel: {channelText}
          </p>
        )}
...
export const SellerPrintSectionHeader: React.FC<{ title: string; rightText?: string }> = ({ title, rightText }) => {
  return (
...
export const SellerPrintAppendixHeader: React.FC<{ title: string; subtitle?: string; pageText?: string; user?: any }> = ({
  title,
  subtitle,
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #000000', paddingBottom: 10, marginBottom: 16 }}>
      <div>
        ...
        <p style={{ margin: '3px 0 0', fontSize: 10.5, color: '#4B5563' }}>
          {subtitle || `Lampiran Standar Metodologi & Rekonsiliasi E-Commerce — ${user?.tenant_name || 'Toko Online'}`}
        </p>
      </div>
...
interface SellerPrintExplanationBoxProps {
  number?: string;
  title: string;
  desc: string;
  formula?: string;
...
export const SellerPrintExplanationBox: React.FC<SellerPrintExplanationBoxProps> = ({
  number,
  title,
  desc,
  formula,
  ...
  return (
    <div 
      className="seller-print-explanation-box" 
      style={{ 
...
interface SellerPrintFooterProps {
  user?: any;
...
export const SellerPrintFooter: React.FC<SellerPrintFooterProps> = ({
  user,
  showSignatures = true,
  authorTitle = 'Dibuat Oleh (Admin Toko / Penjualan Online)',
  approverTitle = 'Disetujui Oleh (Pemilik / Manajer Keuangan)',
}) => {
  ...
  return (
    <div style={{ marginTop: 24, pageBreakInside: 'avoid', breakInside: 'avoid' }}>
      <div 
        className="seller-print-signatures"
        style={{ 
          display: 'grid', 
      ...
      >
        <div>
          ...
          <div style={{ fontWeight: 700, color: '#000000', borderTop: '1px solid #9CA3AF', paddingTop: 6, display: 'inline-block', minWidth: 150 }}>
            {user?.name || 'Staff E-Commerce'}
          </div>
        </div>
        <div>
          <div style={{ color: '#4B5563', marginBottom: 44 }}>{approverTitle},</div>
          <div style={{ fontWeight: 700, color: '#000000', borderTop: '1px solid #9CA3AF', paddingTop: 6, display: 'inline-block', minWidth: 150 }}>
            {user?.tenant_name ? `Manajemen ${user.tenant_name}` : 'Pemilik Bisnis'}
          </div>
        </div>
      </div>
    </div>
