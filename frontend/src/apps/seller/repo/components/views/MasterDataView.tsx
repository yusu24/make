...
import { Database, CheckCircle, Plus, Edit2, Trash2, X, Truck, Info, Store } from 'lucide-react';
...
export const MasterDataView: React.FC<MasterDataViewProps> = ({ stores }) => {
  ...
  const handleSaveSupplier = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const payload = { name: supplierName, contact: supplierContact || null, address: supplierAddress || null };
      if (supplierToEdit) {
        await api.put(`/retail/suppliers/${supplierToEdit.id}`, payload);
      } else {
        await api.post('/retail/suppliers', payload);
      }
      setIsSupplierModalOpen(false);
      fetchData();
    } catch (err: any) {
      setError(err.response?.data?.errors?.name?.[0] || 'Gagal menyimpan supplier.');
    }
  };

  const handleDeleteSupplier = async (s: SupplierRow) => {
    if (!confirm(`Hapus supplier "${s.name}"?`)) return;
    try {
      await api.delete(`/retail/suppliers/${s.id}`);
      fetchData();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      {/* Header */}
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-xs flex items-start justify-between gap-4">
        <div className="flex-1">
          <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            <Database className="w-5 h-5 text-indigo-600 shrink-0" />
            <span className="truncate">Master Data & Integrasi Marketplace API</span>
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 max-w-full">
            Status koneksi channel penjualan, serta data supplier Anda.
          </p>
          <div className="mt-3 flex items-start gap-2 text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 rounded-xl px-3 py-2.5">
            <Info className="w-4 h-4 shrink-0 mt-0.5" />
            <span>Integrasi API Shopee, Tokopedia, dan TikTok Shop belum tersedia. Saat ini hanya toko offline (kasir/POS) yang tercatat otomatis di bawah ini. Klik "Pratinjau Hubungkan Toko" untuk melihat gambaran alur yang nanti akan dijalani.</span>
          </div>
        </div>
        <button
        ...
        >
          ...
          <span className="hidden sm:inline">Pratinjau Hubungkan Toko</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {stores.map((st) => (
          <div key={st.id} className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-xs space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="flex items-center gap-1 text-[11px] font-semibold text-emerald-600 bg-emerald-50 dark:bg-emerald-950/60 px-2 py-0.5 rounded-full">
                <CheckCircle className="w-3 h-3 text-emerald-500" /> Terhubung
              </span>
            </div>

            <div className="font-semibold text-slate-900 dark:text-slate-100 text-sm">{st.name}</div>
            <div className="text-[11px] text-slate-400 font-mono">CODE: {st.storeCode}</div>

            <div className="pt-2 border-t border-slate-100 dark:border-slate-700 text-xs flex items-center justify-between text-slate-500">
              <span>Last Sync: {st.lastSyncAt}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-xs flex items-center justify-between gap-4">
        <div className="flex-1">
          <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2">
            ...
            <span>Database Supplier & Pabrik Vendor</span>
          </h2>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50/30 dark:bg-slate-800/30 text-slate-500 dark:text-slate-400 border-b border-slate-200/80 dark:border-slate-700">
              <tr>
                <th className="px-4 py-3 font-semibold">Nama Supplier / Vendor</th>
