          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-100/60 dark:bg-slate-800/80 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-b border-slate-200/80 dark:border-slate-700">
                <th className="py-3.5 px-4">{i18n?.language === 'en' ? 'DATE / TIME' : 'WAKTU'}</th>
                <th className="py-3.5 px-4">{i18n?.language === 'en' ? 'PRODUCT & SKU' : 'PRODUK & SKU'}</th>
                <th className="py-3.5 px-4">{i18n?.language === 'en' ? 'MOVEMENT TYPE' : 'JENIS MUTASI'}</th>
                <th className="py-3.5 px-4">{i18n?.language === 'en' ? 'QUANTITY' : 'JUMLAH (QTY)'}</th>
                <th className="py-3.5 px-4">{i18n?.language === 'en' ? 'NOTES' : 'CATATAN'}</th>
              </tr>
            </thead>
                ...
                <tr key={mov.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-700/30 transition-colors">
                  ...
                  <td className="py-3.5 px-4">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${
                      ...
                      {mov.type === 'Masuk' ? <ArrowDownLeft className="w-3.5 h-3.5 text-emerald-600" /> : <ArrowUpRight className="w-3.5 h-3.5" />}
