...
export default function RetailAiFab() {
  ...
  const [isHidden, setIsHidden] = useState(() => {
    return localStorage.getItem('bizora_retail_ai_hidden') === 'true';
  });
  ...
  const [position, setPosition] = useState(() => {
    try {
      const saved = localStorage.getItem('bizora_retail_ai_pos');
  ...
  useEffect(() => {
    ...
    const handleToggleAi = (e) => {
      ...
      setIsHidden(next);
      localStorage.setItem('bizora_retail_ai_hidden', String(next));
    };

    window.addEventListener('bizora:open-retail-ai', handleOpenAi);
    window.addEventListener('bizora:toggle-pos-ai', handleToggleAi);
    return () => {
      window.removeEventListener('bizora:open-retail-ai', handleOpenAi);
      window.removeEventListener('bizora:toggle-pos-ai', handleToggleAi);
    };
  }, [isHidden]);

  // Handle pointer down (mouse or touch) for dragging
  const handlePointerDown = (e) => {
    // If clicked on close button, don't drag
    ...
    const onPointerUp = () => {
      ...
      if (isDragging.current) {
        setPosition((curr) => {
          if (curr) localStorage.setItem('bizora_retail_ai_pos', JSON.stringify(curr));
          return curr;
        });
        setTimeout(() => {
          isDragging.current = false;
  ...
  const handleDismiss = (e) => {
    e.stopPropagation();
    setIsHidden(true);
    localStorage.setItem('bizora_retail_ai_hidden', 'true');
  };

  const handleRestore = () => {
    setIsHidden(false);
    localStorage.setItem('bizora_retail_ai_hidden', 'false');
  };
    ...
    <>
      ...
      {isHidden ? (
        <button
          ...
          className="fixed bottom-20 md:bottom-6 right-3 z-40 p-2 rounded-full bg-indigo-600/90 hover:bg-indigo-600 text-white shadow-md backdrop-blur-sm border border-indigo-400/40 transition-all hover:scale-105 active:scale-95"
          title="Tampilkan kembali Bubble AI"
          aria-label="Tampilkan kembali Bubble AI"
        >
        ...
        >
          ...
          {/* Dismiss "X" button - visible everywhere (both mobile & desktop) */}
          <button
            type="button"
            data-dismiss="true"
            ...
            title="Sembunyikan Bubble AI"
            aria-label="Sembunyikan Bubble AI"
          >
            <X size={11} />
          </button>
          ...
          <button
            ...
            className="relative flex items-center justify-center w-[48px] h-[48px] sm:w-[52px] sm:h-[52px] rounded-full bg-gradient-to-tr from-indigo-600 via-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-500/35 hover:shadow-xl hover:shadow-indigo-500/50 hover:scale-105 active:scale-95 transition-transform duration-150 border border-white/20 focus:outline-none"
            title="Buka Retail AI Advisor (Bisa digeser)"
