...
/**
 * Standard utility helpers for Retail PDF Financial Reports
 */
...
/**
 * Reusable Print Header (Kop Surat Resmi Keuangan)
 */
export function RetailPrintHeader({ user, title, subtitle, periodText, startDate, endDate }) {
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2.5px solid #0f172a', paddingBottom: 14, marginBottom: 18 }}>
      <div>
        <h1 style={{ margin: 0, fontSize: 19, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', color: '#0f172a' }}>
          {user?.tenant_name || 'TOKO RETAIL'}
        </h1>
        <p style={{ margin: '4px 0 0', fontSize: 11, color: '#475569', lineHeight: 1.4 }}>
          {user?.address || 'Sistem Kasir & Manajemen Bisnis Retail'} {user?.phone ? `• Telp: ${user.phone}` : ''}
        </p>
        {subtitle && (
 ...
 */
export function RetailPrintSectionHeader({ title, rightText }) {
  return (
 ...
 */
export function RetailPrintAppendixHeader({ title, subtitle, pageText = 'Halaman 2 dari 2', user }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #0F172A', paddingBottom: 10, marginBottom: 16 }}>
      <div>
        ...
        <p style={{ margin: '3px 0 0', fontSize: 10.5, color: '#64748B' }}>
          {subtitle || `Lampiran Metodologi Akuntansi & Sumber Data Transaksi — ${user?.tenant_name || 'Toko Retail'}`}
        </p>
      </div>
 ...
 */
export function RetailPrintExplanationBox({ number, title, desc, formula, variant = 'default' }) {
  const styles = {
  ...
  return (
    <div 
      className="retail-print-explanation-box" 
      style={{ 
...
/**
 * Reusable Print Footer (Signatures & Audit notes)
 */
export function RetailPrintFooter({
  user,
  showSignatures = true,
  authorTitle = 'Dibuat Oleh (Admin / Keuangan)',
  approverTitle = 'Disetujui Oleh (Pimpinan / Pemilik)',
}) {
  ...
  return (
    <div style={{ marginTop: 24, pageBreakInside: 'avoid', breakInside: 'avoid' }}>
      <div 
        className="retail-print-signatures"
        style={{ 
          display: 'grid', 
      ...
      >
        ...
        <div>
          ...
          <div style={{ fontWeight: 700, color: '#0F172A', borderTop: '1px solid #CBD5E1', paddingTop: 6, display: 'inline-block', minWidth: 160 }}>
            {user?.owner_name || user?.tenant_name || 'Pimpinan'}
          </div>
        </div>
      </div>
    </div>
