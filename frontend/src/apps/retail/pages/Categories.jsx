...
export default function Categories() {
  ...
  const addCategory = async (e) => {
    ...
    try { 
      await api.post('/retail/categories', { name: fd.get('name') }); 
      fetchCategories(); 
      e.target.reset(); 
    } catch (e) {}
  };
  ...
  return (
    <div className="retail-page-classic">
      <div className="page-header" style={{ marginBottom: 32 }}>
        <div>
          <h2 className="page-title">Data Master: Kategori Produk</h2>
          <p className="page-sub">Klasifikasi produk untuk mempermudah manajemen stok dan pencarian.</p>
        </div>
      </div>
      ...
      <div className="card table-wrap animate-fade-in">
        <div className="p-6 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <h3 className="font-800 text-lg tracking-tight text-primary-500" style={{ fontFamily: 'var(--font-heading)' }}>Daftar Kategori Produk</h3>
          </div>
        ...
        <table className="table">
          ...
          <tbody>
            {loading ? (
               ...
               <tr><td colSpan="3" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 32 }}>Belum ada data kategori.</td></tr>
            ) : (
              categories.map(c => (
                <tr key={c.id}>
                  <td className="pl-6">
                  ...
                  <td style={{ textAlign: 'right' }} className="pr-6">
                    <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
                      ...
                      <button className="btn btn-sm btn-ghost" style={{ color: 'var(--danger-600)' }} onClick={async () => { if(confirm('Hapus kategori ini?')) { await api.delete(`/retail/categories/${c.id}`); fetchCategories(); } }}><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))
