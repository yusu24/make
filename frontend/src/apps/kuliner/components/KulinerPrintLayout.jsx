...
/**
 * Standard utility helpers for Kuliner / F&B PDF Financial Reports
 */
...
/**
 * Reusable Print Header (Kop Surat Resmi Keuangan Kuliner)
 */
export function KulinerPrintHeader({ user, title, subtitle, periodText, startDate, endDate }) {
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2.5px solid #0f172a', paddingBottom: 14, marginBottom: 18 }}>
      <div>
        <h1 style={{ margin: 0, fontSize: 19, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em', color: '#0f172a' }}>
          {user?.tenant_name || 'RESTORAN & KAFE KULINER'}
        </h1>
        <p style={{ margin: '4px 0 0', fontSize: 11, color: '#475569', lineHeight: 1.4 }}>
          {user?.address || 'Sistem POS Kasir & Manajemen Operasional F&B'} {user?.phone ? `• Telp: ${user.phone}` : ''}
        </p>
        {subtitle && (
      ...
      <div style={{ textAlign: 'right' }}>
        ...
        <p style={{ margin: '6px 0 0', fontSize: 10.5, color: '#64748B' }}>
...
/**
 * Reusable Section Header inside Print Template
 */
export function KulinerPrintSectionHeader({ title, rightText }) {
  return (
...
/**
 * Reusable Appendix / Page 2 Header
 */
export function KulinerPrintAppendixHeader({ title, subtitle, pageText = 'Halaman 2 dari 2', user }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #0F172A', paddingBottom: 10, marginBottom: 16 }}>
      <div>
        ...
        <p style={{ margin: '3px 0 0', fontSize: 10.5, color: '#64748B' }}>
          {subtitle || `Lampiran Metodologi Akuntansi & Prosedur Operasional F&B — ${user?.tenant_name || 'Restoran & Kafe'}`}
        </p>
      </div>
...
/**
 * Reusable Explanation Box for Page 2 Appendix
 */
export function KulinerPrintExplanationBox({ number, title, desc, formula, variant = 'default' }) {
  const styles = {
  ...
  return (
    <div 
      className="kuliner-print-explanation-box" 
      style={{ 
...
/**
 * Reusable Print Footer (Signatures)
 */
export function KulinerPrintFooter({
  user,
  showSignatures = true,
  authorTitle = 'Dibuat Oleh (Admin / Keuangan Resto)',
  approverTitle = 'Disetujui Oleh (Manager / Pemilik)',
}) {
  ...
  return (
    <div style={{ marginTop: 24, pageBreakInside: 'avoid', breakInside: 'avoid' }}>
      <div 
        className="kuliner-print-signatures"
        style={{ 
          display: 'grid', 
      ...
      >
        <div>
          <div style={{ color: '#64748B', marginBottom: 48 }}>{authorTitle},</div>
          <div style={{ fontWeight: 700, color: '#0F172A', borderTop: '1px solid #CBD5E1', paddingTop: 6, display: 'inline-block', minWidth: 160 }}>
            {user?.name || 'Staf Keuangan F&B'}
          </div>
        </div>
        <div>
          <div style={{ color: '#64748B', marginBottom: 48 }}>{approverTitle},</div>
          <div style={{ fontWeight: 700, color: '#0F172A', borderTop: '1px solid #CBD5E1', paddingTop: 6, display: 'inline-block', minWidth: 160 }}>
            {user?.owner_name || user?.tenant_name || 'Pemilik / Manager'}
          </div>
        </div>
      </div>
    </div>
