...
export const PrintSpkModal: React.FC<PrintSpkModalProps> = ({ order, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/60 backdrop-blur-xs flex items-end sm:items-center justify-center sm:p-5">
      <div className="bg-white border border-slate-200 rounded-t-3xl sm:rounded-3xl w-full max-w-3xl max-h-[95vh] flex flex-col shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-150">
                ...
                {order.dpAmount ? (
                  <>
                    <tr className="border-t border-slate-300">
                      <td colSpan={3} className="p-2 text-right text-slate-600 font-medium">UANG MUKA (DP):</td>
                      <td className="p-2 text-right text-emerald-600 text-sm font-semibold">-{formatRupiah(order.dpAmount)}</td>
                    </tr>
                    <tr className="border-t border-slate-300">
                      <td colSpan={3} className="p-2 text-right font-bold text-slate-800">SISA TAGIHAN:</td>
                      <td className="p-2 text-right text-rose-600 text-sm font-bold">{formatRupiah(order.grandTotal - order.dpAmount)}</td>
                    </tr>
                  </>
                ) : null}
              </tfoot>
