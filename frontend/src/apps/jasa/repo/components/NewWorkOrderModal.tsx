  ...
  const handleAddPart = (e: React.MouseEvent) => {
    ...
    setParts([
      ...parts,
      {
        ...
        quantity: parseNumberInput(partQtyInput) || 1,
        unitCost: parseNumberInput(partCostInput) || 0
      }
    ]);
    setPartNameInput('');
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <select
                  ...
                  aria-label="Pilih pekerja/tim yang ditugaskan"
                  className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-slate-900 font-semibold focus:ring-2 focus:ring-blue-500 focus:outline-none text-xs shadow-2xs"
                >
                  {technicians.map(t => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  value={laborRate}
                  onChange={(e) => setLaborRate(formatNumberInput(e.target.value))}
                  className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-slate-900 shadow-2xs font-medium"
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Uang Muka (DP) (Rp)</label>
                <input
                  type="text"
                  placeholder="Contoh: 100.000"
                  value={dpAmountInput}
                  onChange={(e) => setDpAmountInput(formatNumberInput(e.target.value))}
                  className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-slate-900 shadow-2xs font-medium"
                />
              </div>
            </div>
            ...
            <div className="pt-2">
              ...
              <div className="flex flex-wrap gap-2 mb-2.5 relative">
                <div className="flex-1 min-w-[200px]">
                  <select
                    value={isCustomPart ? 'custom' : partNameInput}
                    onChange={(e) => {
                      if (e.target.value === 'custom') {
                        ...
                        setPartNameInput(e.target.value);
                        if (selected) {
                          setPartCostInput(formatNumberInput(selected.price.toString()));
                        }
                      }
                    }}
                    className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-slate-900 shadow-2xs appearance-none text-xs"
                  >
                ...
                {isCustomPart && (
                  <input
                    ...
                    className="flex-1 min-w-[150px] bg-white border border-slate-200 rounded-xl px-3.5 py-2 text-xs shadow-2xs"
                  />
                )}
                <input
                  type="number"
                ...
                <input
                  ...
                  value={partCostInput}
                  onChange={(e) => setPartCostInput(formatNumberInput(e.target.value))}
                  className="w-28 bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 shadow-2xs"
                />
                <button
                  type="button"
                  onClick={handleAddPart}
                  className="px-3.5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-xl flex items-center space-x-1 shadow-2xs cursor-pointer"
                >
