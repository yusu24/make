...
use App\Models\User;
use App\Http\Controllers\Api\Kuliner\KulinerBackupController;

class SendTenantKulinerBackups extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'kuliner:auto-backup {--force : Force send backup regardless of schedule/last run time}';
     ...
     * @var string
     */
    protected $description = 'Generate and send scheduled automatic backups (Excel / JSON) via email to Kuliner/F&B tenant owners.';
    ...
    {
        $this->info('Memulai proses backup otomatis untuk tenant Kuliner (F&B)...');

        $force = $this->option('force');
        $now   = Carbon::now();

        $settings = KulinerSetting::where('auto_backup_enabled', true)->get();

        if ($settings->isEmpty()) {
            $this->info('Tidak ada tenant Kuliner dengan backup otomatis aktif.');
            return 0;
        }
        ...
        foreach ($settings as $setting) {
            ...
            $storeName = $setting->store_name ?? $tenant->name ?? 'Restoran';
            ...
            try {
                if ($format === 'excel' || $format === 'xlsx') {
                    ...
                    Mail::raw(
                        "Halo,\n\nTerlampir adalah file Backup Otomatis Terjadwal ({$setting->auto_backup_frequency}) dalam format Microsoft Excel (.xlsx) untuk restoran {$storeName} (ID: {$tenantId}) yang dibuat pada {$now->translatedFormat('d F Y H:i')} WIB.\n\nFile ini berisi beberapa Sheet:\n- Menu Makanan & Minuman\n- Riwayat Pesanan & Transaksi\n- Stok Bahan Baku & Inventaris\n- Data Supplier\n- Daftar Meja\n- Profil Restoran\n\nFile dapat dibuka langsung di Microsoft Excel atau Google Sheets.\n\nSalam,\nTim Bizora Platform",
                    ...
                    $tempPath    = 'temp/' . $filename;
                    $data        = KulinerBackupController::getBackupDataForTenant($tenantId);
                    $jsonContent = json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);

                    Storage::disk('local')->put($tempPath, $jsonContent);

                    Mail::raw(
                        "Halo,\n\nTerlampir adalah file Backup Otomatis Terjadwal ({$setting->auto_backup_frequency}) dalam format JSON untuk restoran {$storeName} (ID: {$tenantId}) yang dibuat pada {$now->translatedFormat('d F Y H:i')} WIB.\n\nSimpan file ini sebagai arsip data toko Anda.\n\nSalam,\nTim Bizora Platform",
                        function ($message) use ($email, $tempPath, $filename, $storeName, $now) {
                            $message->to($email)
